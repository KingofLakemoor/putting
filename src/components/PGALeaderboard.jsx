...
const PGALeaderboard = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeRounds = onSnapshot(collection(db, ROUNDS_KEY), (snapshot) => {
      ...
      currentActive.sort((a, b) => new Date(b.date || 0).getTime() - new Date(a.date || 0).getTime());
  ...
  const livePlayers = useMemo(() => {
    ...
    activeRounds.forEach((r) => {
      ...
      if (r.opponent_id) {
        ...
        const oppScores = [];
        for (let i = 1; i <= 18; i++) {
            const score = r.opponent_scores && r.opponent_scores[i] ? r.opponent_scores[i] : null;
            const holeData = holesMap.get(i);
            const par = holeData ? holeData.par : 2; // Default par 2
        ...
        <table className="w-full border-collapse text-center">
            <thead className="bg-slate-800 text-slate-400 text-xs uppercase tracking-wider border-b border-slate-700">
                <tr>
                    ...
                    {[...Array(18)].map((_, i) => (
                        <th key={i} className="py-3 px-1 font-bold w-8">{i + 1}</th>
                    ))}
            ...
            <tbody className="divide-y divide-slate-800/50">
                {livePlayers.length === 0 ? (
                    <tr>
                        <td colSpan="23" className="py-12 text-slate-500 italic text-center">
                    ...
                    livePlayers.map((player, index) => (
                        ...
                        >
                            ...
                            <td className="py-3 px-2 font-data text-slate-400">
                                {player.holesPlayed === 18 ? 'F' : player.holesPlayed === 0 ? '-' : player.holesPlayed}
                            </td>
                            <td className="py-3 px-2 font-data font-bold">
                                <span className={player.relativeScore > 0 ? 'text-red-500' : player.relativeScore < 0 ? 'text-kelly-green' : 'text-slate-300'}>
                            ...
                            {player.scores.map((scoreObj, i) => (
                                <td key={i} className="py-3 px-1 font-data text-sm">
                                    <div className={getScoreStyle(scoreObj.score, scoreObj.par)}>
