...
const PuttingDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        try {
          ...
          if (scores.length > 0) {
            ...
            for (const course of courses) {
              courseHolesMap[course.course_id] = course.holes
                ? course.holes.length
                : 18; // default to 18 if no holes
            }
            ...
            for (const score of scores) {
              ...
              const holes =
                courseId && courseHolesMap[courseId]
                  ? courseHolesMap[courseId]
                  : 18;
              ...
              const scoreDate = score.timestamp
                ? new Date(score.timestamp).getTime()
                : roundObj && roundObj.date
                  ? new Date(roundObj.date).getTime()
                  : 0;
            ...
            if (totalHoles > 0) {
              ...
              if (previousTotalHoles > 0) {
                const previousAvg =
                  (previousTotalScore / previousTotalHoles) * 18;
      ...
      try {
        ...
        const activeEvents = rounds.filter(
          (r) =>
        ...
        if (roundsForStandings.length > 0) {
          ...
          roundsForStandings = rounds.filter(
            (r) => r.season === settings.live_season,
          );
  ...
  const handleSelectEventRound = async (eventRound) => {
    ...
    try {
      if (activeRoundId) {
        setError(
          "You already have an active round. Please complete or discard it before starting a new one.",
        );
      ...
      if (eventRound.score_limit) {
        ...
        const scoresForEvent = historicalScores.filter(
          (s) => s.round_id === eventRound.round_id,
        );
        if (scoresForEvent.length >= eventRound.score_limit) {
          setError(
            `You have reached the limit of ${eventRound.score_limit} score(s) for this event.`,
          );
      ...
      const newRound = await createActiveRound(
        currentUser.uid,
        userName,
        eventRound.round_id,
        eventRound.name,
        eventRound.course_id,
      );
  ...
  return (
    <div className="min-h-screen bg-dark-bg text-white p-4 md:p-8 font-sans">
      ...
      <header className="flex justify-between items-center mb-8">
        <div>
          <h1 className="font-sports text-4xl tracking-wide uppercase">
            Club 602
          </h1>
          <p className="text-slate-400 text-sm font-data">
            LEAGUE MANAGEMENT v2.0
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className="h-2 w-2 bg-kelly-green rounded-full animate-ping" />
          <span className="text-xs uppercase font-bold tracking-widest text-kelly-green">
            Live Session
          </span>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 auto-rows-[180px]">
            ...
            {dashboardStandings.slice(0, 5).map((player, index) => {
              ...
              return (
                <div
                  ...
                  className="flex items-center justify-between p-4 bg-dark-bg/50 rounded-xl border-l-4 border-kelly-green group-hover:translate-x-1 transition-transform"
                >
                  <div className="flex items-center gap-4">
                    <span className="font-data font-black text-2xl italic text-slate-600">
                      0{rank}
                    </span>
                    <div>
                    ...
                    <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                      {activeEventRounds.length === 0 ? (
                        <p className="text-slate-400 text-sm">
                          No active rounds available right now.
                        </p>
                      ) : (
                        activeEventRounds.map((round) => {
                          ...
                          return (
                            ...
                            >
                              <div>
                                <h3
                                  className={`font-bold text-lg transition-colors ${!isSubmitting ? "group-hover:text-kelly-green" : ""}`}
                                >
                                  {round.name || "Round"}
                                </h3>
                              ...
                              {isSubmitting ? (
                                ...
                                <PlusCircle
                                  className="text-slate-500 group-hover:text-kelly-green transition-colors"
                                  size={20}
                                />
                              )}
                            </button>
        ...
        <div className="order-4 md:order-none bg-dark-surface border border-slate-700/50 rounded-2xl p-6 flex flex-col justify-between relative">
          ...
          <div className="text-5xl font-data font-black text-white">
            {myAvg}
          </div>
          {avgTrend && (
            <div
              className={`${trendColor} text-xs flex items-center gap-1 mt-2`}
            >
              {trendIcon} {avgTrend}
            </div>
          )}
          <Link
            to="/history"
            className="absolute bottom-6 right-6 text-[10px] text-slate-500 hover:text-white transition-colors uppercase font-bold tracking-widest"
          >
            More Stats
          </Link>
        </div>
        ...
        <div className="order-5 md:order-none md:col-span-3 bg-dark-surface border border-slate-700/50 rounded-2xl p-6">
          ...
          {activeEventRounds.length === 0 ? (
            <p className="text-slate-400">
              Have an idea for a putting league? Let Club 602 know on{" "}
              <a
                href="https://www.instagram.com/club_602/"
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
              {activeEventRounds.map((round) => {
                ...
                return (
                  <div
                    key={round.round_id}
                    className="min-w-[280px] bg-dark-bg p-4 rounded-xl border border-slate-800 flex justify-between items-center shrink-0"
                  >
                    <div>
                      <p className="font-bold">
                        {round.name ? round.name : "Round"}
                      </p>
