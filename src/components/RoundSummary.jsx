...
const RoundSummary = ({ roundData, onFinalize, onDiscard, isPB, isSubmitting, isSubmitted }) => {
  ...
  return (
    <div className="min-h-screen bg-dark-bg text-white p-6 flex flex-col font-sans">
      ...
      >
        ...
        <h1 className="font-sports text-4xl uppercase tracking-tight">Round Complete</h1>
        <p className="text-slate-500 font-data text-xs mt-1">{locationStr} • {dateStr}</p>
      </motion.div>

      {/* Hero Stats Section */}
      <div className={`grid gap-4 mb-8 ${roundData.opponent_id ? 'grid-cols-3' : 'grid-cols-2'}`}>
        <div className="bg-dark-surface border border-slate-800 p-4 sm:p-6 rounded-2xl text-center relative overflow-hidden">
          ...
          <p className={`text-[9px] sm:text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-1 ${isPB ? 'mt-3' : ''}`}>Your Score</p>
          <p className="text-4xl sm:text-5xl font-data font-black text-white">{totalScore}</p>
        </div>

        {roundData.opponent_id && (
          <div className="bg-dark-surface border border-slate-800 p-4 sm:p-6 rounded-2xl text-center">
            <p className="text-[9px] sm:text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-1">Opponent</p>
            <p className="text-4xl sm:text-5xl font-data font-black text-white">{opponentTotalScore}</p>
          </div>
        )}

        <div className="bg-dark-surface border border-slate-800 p-4 sm:p-6 rounded-2xl text-center">
          <p className="text-[9px] sm:text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-1">Birdies</p>
          <p className="text-4xl sm:text-5xl font-data font-black text-kelly-green">{birdies}</p>
        </div>
      </div>

      {/* Hole-by-Hole Mini Grid */}
      <div className="bg-dark-surface border border-slate-800 rounded-2xl p-4 mb-8">
        <h3 className="text-[10px] text-slate-500 font-bold uppercase mb-4 text-center">Scorecard Detail</h3>
        <div className="grid grid-cols-3 gap-3">
          {Object.entries(roundData.scores || {}).map(([hole, score]) => (
            <div key={hole} className="flex flex-col items-center bg-dark-bg/50 p-3 rounded-xl border border-slate-800">
              <span className="text-[10px] text-slate-600 font-bold">H{hole}</span>
              <span className={`text-xl font-data font-bold ${score < 3 ? 'text-kelly-green' : 'text-white'}`}>
                {score}
              </span>
            </div>
          ))}
      ...
      <div className="mt-auto space-y-4">
        <button
          ...
          className={`w-full py-5 rounded-2xl font-bold text-lg flex items-center justify-center gap-2 transition-all duration-300 ${
            isSubmitting
              ? 'bg-kelly-green/50 text-dark-bg/50 cursor-not-allowed shadow-none'
              : 'bg-kelly-green text-dark-bg shadow-[0_0_30px_rgba(76,187,23,0.2)] hover:bg-kelly-green/90 hover:scale-[1.02]'
          }`}
        >
          <Trophy size={20} className={isSubmitting ? 'animate-pulse' : ''} />
          {isSubmitting ? 'SUBMITTING...' : isSubmitted ? 'RETURN TO HOME' : 'SUBMIT TO LEADERBOARD'}
        </button>
              ...
              <div className="flex min-h-full items-center justify-center p-4">
                ...
                >
                  ...
                  <Dialog.Description className="text-slate-400 text-sm mb-8">
                    Are you sure you want to discard this round? This action cannot be undone and your score will not be saved.
                  </Dialog.Description>
