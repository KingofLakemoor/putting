...
const LeagueStandings = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeCupPoints = onSnapshot(
  ...
  const calculateRankings = useCallback(
    (targetDateMs = null, filterParam = "602_cup") => {
      let currentRounds = rounds;
      let currentScores = scores;

      // Filter by date for historical calculation
      if (targetDateMs) {
        currentRounds = rounds.filter(
          (r) =>
            new Date(r.date || r.createdAt || Date.now()).getTime() <=
            targetDateMs,
        );
        const validRoundIds = new Set(currentRounds.map((r) => r.round_id));
        currentScores = scores.filter(
          (s) =>
            validRoundIds.has(s.round_id) &&
            new Date(s.timestamp).getTime() <= targetDateMs,
        );
      }
      ...
      if (filterParam === "this_month") {
        ...
        if (liveSeason) {
          ...
          const activeRounds = currentRounds.filter(
            (r) => (r.status || "").toLowerCase() === "active",
          );
          if (activeRounds.length > 0) {
            const latestActiveRound = activeRounds.sort(
              (a, b) => new Date(b.date || 0) - new Date(a.date || 0),
            )[0];

            if (latestActiveRound.season) {
              currentRounds = currentRounds.filter(
                (r) => r.season === latestActiveRound.season,
              );
            } else {
              currentRounds = currentRounds.filter(
                (r) => r.round_id === latestActiveRound.round_id,
              );
            }
      ...
      if (filterParam === "602_cup") {
        ...
        const currentCupPoints = cupPoints.filter(
          (cp) => cp.year === currentYear,
        );
      ...
      for (const score of currentScores) {
        ...
        if (!player) {
          ...
          if (
            round &&
            round.player_name &&
            round.player_id === score.player_id
          ) {
      ...
      const orphanIds = Object.keys(scoresByPlayerId).filter(
        (id) => !knownPlayerIds.has(id),
      );
      ...
      const orphanStats = orphanIds.map((id) => {
        ...
        const rawName =
          roundWithPlayer && roundWithPlayer.player_name
            ? roundWithPlayer.player_name
            : "Unknown Player";

        return calculateStats(id, rawName, playerScores);
      });

      const activePlayers = [...playerStats, ...orphanStats].filter(
        (p) => p.played > 0,
      );
  ...
  const getTrendIcon = (trend) => {
    if (trend === "up")
      return <ChevronUp className="text-kelly-green" size={16} />;
    if (trend === "down")
    ...
    <div className="bg-dark-bg p-1 lg:p-6 text-white font-sans w-full max-w-2xl mx-auto">
      ...
      <div className="flex items-end justify-between mb-8 border-b border-slate-800 pb-4">
        ...
        <div className="text-right">
          <span className="bg-slate-800 text-slate-400 text-[10px] px-2 py-1 rounded font-bold uppercase">
            Live Updates
          </span>
        </div>
      </div>
          ...
          >
            ...
            <div className="flex items-center gap-5">
              <div className="relative">
                <span
                  className={`font-sports text-3xl italic tracking-tighter ${index < 3 ? "text-white" : "text-slate-600"}`}
                >
              ...
              <div>
                <p
                  className={`font-bold text-lg uppercase tracking-tight ${index === 0 ? "text-kelly-green" : "text-white"}`}
                >
                  {formatDisplayName(player.name, players)}
                </p>
                <div className="flex items-center gap-2">
                  {getTrendIcon(player.trend)}
                  <span className="text-[10px] text-slate-500 font-data uppercase">
                    Pos Change
                  </span>
                </div>
              </div>
