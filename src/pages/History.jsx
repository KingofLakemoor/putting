...
const History = () => {
  ...
  useEffect(() => {
    const fetchHistoryData = async () => {
      ...
      try {
        ...
        const [playerScores, allRounds, courses, allScores, allPlayers] =
          await Promise.all([
  ...
  const processData = (
    ...
    const currentUserActualId = currentUserProfile
      ? currentUserProfile.player_id
      : currentUser.uid;
    ...
    for (const score of playerScores) {
      ...
      const sortedScores = [...roundScores].sort(
      ...
      const playerRank =
        sortedScores.findIndex((s) => {
          const p = playersMap.get(s.player_id);
          const actualId = p ? p.player_id : s.player_id;
          return (
            actualId === currentUserActualId || actualId === currentUser.uid
          );
        }) + 1;
        ...
        >
          ...
          <div className="bg-dark-surface border border-slate-700/50 rounded-2xl p-6 flex flex-col justify-between relative shadow-lg">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <Activity size={16} className="text-kelly-green" /> Career Avg (18
              Holes)
            </h3>
            <div className="text-5xl font-data font-black text-white mt-4">
              {historyData.stats.averageScore}
            </div>
            <p className="text-[10px] text-slate-500 mt-2 uppercase tracking-tighter">
              Total Rounds: {historyData.stats.totalRounds}
            </p>
          </div>
              ...
              <div className="space-y-4">
                {historyData.personalBests.map((pb) => (
                  ...
                  >
                    ...
                    <div className="text-2xl font-data font-bold text-kelly-green">
                      {pb.score}
                    </div>
                  </div>
                ))}
          ...
          {historyData.rounds.length === 0 ? (
            ...
            <div className="space-y-3">
              {historyData.rounds.map((round, idx) => {
                ...
                const monthStr = validDate
                  ? d.toLocaleString("en-US", { month: "short" })
                  : "---";
                const dayStr = validDate
                  ? d.toLocaleString("en-US", { day: "2-digit" })
                  : "--";
                return (
                  <div
                    key={round.id}
                    className="group flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-dark-bg/50 rounded-xl border border-slate-800 hover:border-kelly-green/50 transition-colors gap-4 sm:gap-0"
                  >
                    <div className="flex items-center gap-4">
                      <div className="bg-slate-800/50 p-3 rounded-lg flex flex-col items-center justify-center min-w-[60px]">
                        <span className="text-xs text-slate-400 font-data uppercase">
                          {monthStr}
                        </span>
                      ...
                      <div>
                        <p className="font-bold text-white uppercase text-sm sm:text-base">
                          {round.roundName || round.courseName}
                        </p>
                        <p className="text-[10px] sm:text-xs text-slate-500 flex items-center gap-1 uppercase tracking-wider mt-1">
                          <MapPin size={10} className="text-kelly-green" />{" "}
                          {round.courseName}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-6 sm:gap-8 border-t border-slate-800 pt-3 sm:border-0 sm:pt-0">
                      <div className="text-center">
                        ...
                        <div className="flex items-center gap-1 justify-center">
                          {round.rank === 1 && (
                            <Medal size={12} className="text-kelly-green" />
                          )}
                          <p
                            className={`text-lg font-data font-bold ${round.rank === 1 ? "text-kelly-green" : "text-slate-300"}`}
                          >
                            {round.rank}
                          </p>
                      ...
                      <div className="text-center sm:border-l border-slate-800 sm:pl-6">
                        ...
                        <p className="text-2xl font-data font-black text-white">
                          {round.score}
                        </p>
                      </div>
                    </div>
