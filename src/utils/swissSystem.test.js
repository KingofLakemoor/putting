...
describe("Swiss System Pairings", () => {
  ...
  it("does not assign a bye to a player who has already had one", () => {
    ...
    const player3Pairing = pairings.find(
      (p) => p.player1_id === "3" || p.player2_id === "3",
    );
  ...
  it("avoids repeat matchups and finds an alternative valid pairing", () => {
    ...
    const previousMatchups = [{ player1_id: "1", player2_id: "2" }];
    ...
    const player1Pairing = pairings.find(
      (p) => p.player1_id === "1" || p.player2_id === "1",
    );
    expect(
      player1Pairing.player1_id === "2" || player1Pairing.player2_id === "2",
    ).toBe(false);
  });
  ...
  it("throws an error if a valid pairing is impossible", () => {
    ...
    expect(() => generateSwissPairings(players, previousMatchups)).toThrowError(
      /Could not generate valid Swiss pairings/,
    );
  });
});
